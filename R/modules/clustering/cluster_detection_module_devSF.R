# Version 3
# Updated: 2025-04-09
# Authors: Rebecca Goldberg, rgoldberg2@health.nyc.gov
#          Steffen Foerster, sfoerster@health.nyc.gov
# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

# Changes from version 2
# - modified do_hdbscan function to output original polygons with cluster assignments
...
clusterUI <- function(id) {
  ...
  tagList(
    actionButton(ns("calculate"), "Calculate Clusters"),
    sliderInput(ns("minpts"), "Minimum Number of Points", min = 1, max = 100, value = 10)
  )
}
...
clusterServer <- function(id, selected_layers, sf_layers_list, boundary) {
  moduleServer(id, function(input, output, session) {
    ...
    observe({
      print("Inputs visible in module right now:")
      print(names(input))
    })
    ...
    do_hdbscan <- function(sf_layers, boundary, minPts = 10) {
      ...
      # Rasterize all input layers (field_list must match the sf_layers list length)
      ...
      # Standardize each raster layer before stacking
      ...
      # v3 addition: assign original polygons a cluster ID
      cluster_raster <- raster_template
      values(cluster_raster) <- NA
      
      # Find non-NA cell positions using one of the sample rasters
      valid_cells <- which(!is.na(values(raster_stack[[1]])))
      ...
      # Return cluster results as df as well as the polygons
      output <- list(raster_data, 
                     fishnet_with_values, 
                     boundary) # original boundary layer with cluster_id added for visualization
      return(output)
    }
