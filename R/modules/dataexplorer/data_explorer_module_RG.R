...
outcomeUI <- function(id) {
  ns <- NS(id)
  shinyTree(ns("outcome_tree"), theme="proton", themeIcons = FALSE, themeDots = FALSE, checkbox = TRUE, search = TRUE,
            types = "{ 'default': { 'a_attr' : { 'style' : 'font-size:16px;' }}}"
  )
}

outcomeServer <- function(id,disease_list){
  moduleServer(id, function(input, output, session) {
    ns <- session$ns
    
    outputSelection <- reactiveVal(character(0))
    
    
    outcome_data <- reactive({
      req(disease_list())
      generate_jstree_data(disease_list(),"Disease")
    })
    ...
    observeEvent(input$outcome_tree, {
      tree.sel <- get_selected(input$outcome_tree, format = c("names"))
      selections <- unlist(tree.sel)[tree.sel %notin% names(input$outcome_tree)]
      outputSelection(as.character(selections))
    })
...
mapVizUI <- function(id) {
  ...
  tagList(
    ...
    tags$script(HTML(sprintf("
     Shiny.addCustomMessageHandler('initLayerTracker', function(groups) {
        var map = $('#%s').data('leaflet-map');
        ...
        groups.forEach(function(group) {
          map.on('overlayadd', function(e) {
            if (e.name === group) {
              ...
              Shiny.setInputValue('%s', lastActiveGroup, {priority: 'event'});
            }
          });
        });
      });
    ", ns("map_explorer"), ns("last_selected_group"))))
  )
}
...
mapVizServer <- function(id, selected_layers, data_layers, base_layers, variable = "percent_all") {
  moduleServer(id, function(input, output, session) {
    ...
    output$map_explorer <- renderLeaflet({
      ...
      map <- leaflet(options = leafletOptions(minZoom = 11, maxZoom = 14)) %>%
    ...
    observe({
      session$sendCustomMessage("initLayerTracker", names(data_layers))
    })
    ...
    observeEvent(input$map_explorer_groups, {
      ...
      if(length(input$map_explorer_groups[input$map_explorer_groups %notin% c("Borough", "ZCTA", "UHF Neighborhood", "Census Tract")]) > 2){
        showNotification("Please select up to two layers to visualize",type = "error",duration = 2)
      }
      
      if(length(input$map_explorer_groups[input$map_explorer_groups %notin% c("Borough", "ZCTA", "UHF Neighborhood", "Census Tract")]) == 1 ){
        ...
        if(length(active_polygon_layers) != 0) {
          ...
          print(paste("Most recent polygon layer: ",most_recent_polygon_layer))
  
          # Generate a palette for the legend
          pal <- colorNumeric(palette = "YlGnBu", domain = data_layers[[most_recent_polygon_layer]][[variable]]
    ...
    observe({
      ...
      for (layer_name in layers_to_add) {
       ...
       # dat <<- c(data_layers,outcome_data())
        print(6)

       # print(names(data_layers))
        ...
        if (sf::st_geometry_type(layer_data)[1] == "POINT") {
          ...
          pal <- colorNumeric(palette = "YlGnBu", domain = layer_data[[variable]], na.color = "transparent")
      ...
      if ((length(selection) > 0 & sum(selection == "") == 0) ) {
        ...
        most_recent_layer <- c(selection[[length(selection)]])
        ...
        map_proxy() %>%
          addLayersControl(
            ...
            overlayGroups = c("Borough", "ZCTA", "UHF Neighborhood", "Census Tract",selection), #[selection %notin% names(level_data)]),
            # baseGroups = selection[selection %in% names(level_data)],
            options = layersControlOptions(collapsed = FALSE)
          )
